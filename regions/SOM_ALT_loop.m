    ...
    for i_ngb=1:2
        ...
        for n=[2,10]
            for sig=[2,1]
                sigma = [sig,1];
                for inorm=1:2
                    norm = char(norms(inorm));
                    fprintf(num2str(location))
                    location=location+1;
                   
                    ncname = strcat('/home/cmachado/data/som_output/', 'som_',num2str(x),'x',num2str(y), '_',file, '_sig_init',num2str(sig),'_norm_',norm, '_train_n',num2str(n),'_ngb_function_',ngb_function,'_.nc');
                    fprintf(ncname)
                    % % =============================================================
                    %   PREP DATA
